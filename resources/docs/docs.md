...
The `home-routes` handler in the `weather-7.routes.home` namespace
...
The `home-page` function will in turn call the `weather-7.layout/render` function
...
The rest of this page is rendered by ClojureScript found in the `src/cljs/weather_7/core.cljs` file.



#### Organizing the routes

The routes are aggregated and wrapped with middleware in the `weather-7.handler` namespace:
...
Request middleware functions are located under the `weather-7.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `weather-7.dev-middleware` namespace found in
